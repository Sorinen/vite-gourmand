...
from app.crud.avis import get_all_avis, get_avis_by_id, create_avis, delete_avis
...
def create_new_avis(avis: AvisCreate, db: Session = Depends(get_db)):
    return create_avis(db, avis)

@router.delete("/{avis_id}")
def remove_avis(avis_id: int, db: Session = Depends(get_db)):
    avis = delete_avis(db, avis_id)
    if not avis:
        raise HTTPException(status_code=404, detail="Avis introuvable")
    return {"message": "Avis supprimé avec succès"}

@router.patch("/{avis_id}/valider")
def valider_avis(
    avis_id: int,
